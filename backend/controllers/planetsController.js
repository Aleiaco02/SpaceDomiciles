...
export function store(req, res) {
  ...
  const sql = `
    INSERT INTO planets 
    (id_galaxy, name, planet_size, temperature_min, temperature_max, population,
     surface_available, distance_from_earth, description, image)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;
  ...
  connection.query(
    ...
    (err, result) => {
      if (err)
        return res
          .status(400)
          .json({ error: "Insert failed", sqlError: err.sqlMessage });
...
export function update(req, res) {
  ...
  connection.query(
    ...
    (err, result) => {
      if (err)
        return res.status(500).json({ error: "Update failed", sqlError: err.sqlMessage });
